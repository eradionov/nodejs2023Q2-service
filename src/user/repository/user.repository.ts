...
export class UserRepository extends AbstractRepository<User> {
  findOneByName(login: string): User | undefined {
    return <User | undefined>AbstractRepository.users.find((user) => user.login === login);
  }

  findOneById(id: string): User | undefined {
    return <User | undefined>AbstractRepository.users.find((user) => user.id === id);
  }
  ...
  remove(id: string) {
    const userIndex = AbstractRepository.users.findIndex((user) => user.id === id);

    if (userIndex === -1) {
      throw new EntityNotExistsException(id);
    }

    AbstractRepository.users = AbstractRepository.users.filter((user) => user.id !== id);
  }
}
