...
export class ArtistRepository extends AbstractRepository<Artist> {
    public findOneByName(name: string): Artist | undefined {
        return <Artist | undefined>AbstractRepository.artists.find((artist) => artist.name === name);
    }

    public findOneById(id: string): Artist | undefined {
        return <Artist | undefined>AbstractRepository.artists.find((artist) => artist.id === id);
    }
    ...
    remove(id: string) {
        const userIndex = AbstractRepository.artists.findIndex((user) => user.id === id);

        if (userIndex === -1) {
            throw new EntityNotExistsException(id);
        }

        AbstractRepository.artists = AbstractRepository.artists.filter((artist) => artist.id !== id);
    }
}